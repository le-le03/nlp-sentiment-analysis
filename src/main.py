...
def main():
    ...
    try:
        ...
        # 打印数据信息
        print("\n数据加载完成:")
        print(f"  训练集: {data_info['train_samples']:,} 条")
        print(f"  验证集: {data_info['val_samples']:,} 条")
        print(f"  测试集: {data_info['test_samples']:,} 条")
        print(f"  正面比例: {data_info['positive_ratio']:.1%}")
        print(f"  词汇表: {data_info['vocab_size']:,}")
        print(f"  特征数: {data_info['num_features']:,}")
        ...
        try:
            ...
            joblib.dump(data_loader.tokenizer, 'models/tokenizer.pkl')
            joblib.dump(data_loader.scaler, 'models/scaler.pkl')
            joblib.dump(data_loader.vectorizer, 'models/vectorizer.pkl')
            print("预处理组件已保存到 models/")
        ...
        for i, text in enumerate(demo_texts, 1):
            positive_kw = ["热情", "干净", "舒适", "推荐", "超值", "不错", "满意"]
            negative_kw = ["脏", "吵", "贵", "失望", "差", "敷衍", "恶心", "垃圾"]
            if any(w in text for w in positive_kw):
                sentiment = "正面"
            elif any(w in text for w in negative_kw):
                sentiment = "负面"
            else:
                sentiment = "中性"
